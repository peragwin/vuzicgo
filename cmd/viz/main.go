...
func main() {
	...
	fftProc, err := fft.NewMultiRateFFT(minFrameSize, maxFrameSize, false)
	if err != nil {
		panic(err)
	}
	fftChans := fftProc.Process(done, source64)

	specChans := make([]chan []float64, len(fftChans))
	for i := range fftChans {
		proc := new(fft.PowerSpectrumProcessor)
		specChans[i] = proc.Process(done, fftChans[i])
	}
	...
	fsOut := f.ProcessGroup(done, specChans, minFrameSize, maxFrameSize)
